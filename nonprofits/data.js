...
module.exports = {
  ...
  nonprofits: {
    source: `${
      ...
      config.data ? config.data.publishYear.data : '2019'
...
function parseNonprofits(input) {
  ...
  let publishYear = config.data ? config.data.publishYear.data : 2019;
  let validYears = [publishYear, publishYear - 1];
  let parsed = input.objects;

  // Ohh, this should be the 2018 list ...
  let list2018 = JSON.parse(
    fs.readFileSync(
      path.join(__dirname, '../sources/2018-non-profit-list.json')
    )
  );
  ...
  // Rank previous year
  parsed = _.map(parsed, p => {
    let previous = _.find(list2018, { coid: p.coid });
    if (previous) {
